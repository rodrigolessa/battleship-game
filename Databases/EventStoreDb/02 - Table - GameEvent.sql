CREATE TABLE [eventstore].[GameEvent] (
    ...
    [EventSequence] INT NOT NULL, -- Optimistic concurrency control (OCC). Concurrency token. To avoid dirty read and other problem of Race Conditions, we only persist data if the sequence is the same as we read before. Conditional UPDATE and in this case will be a conditional INSERT
...
ALTER TABLE [eventstore].[GameEvent]
    ADD CONSTRAINT UQ_Aggregate_Sequence UNIQUE (AggregateId, EventSequence);
